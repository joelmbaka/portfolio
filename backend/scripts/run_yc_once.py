...
async def main() -> None:
    ...
    export_dir = Path(settings.yc_export_dir)
    latest_json = export_dir / "latest.json"
    previous_job_ids = read_previous_job_ids("yc", latest_json)

    jobs = await scrape_yc_roles(ROLE_URLS)
    rows = [job.to_dict() for job in jobs if ("yc", job.job_id) not in SUPPRESSED_JOB_KEYS]
    current_job_ids = {str(row["job_id"]) for row in rows if row.get("job_id")}
    new_job_ids = current_job_ids - previous_job_ids

    json_path = export_dir / f"yc_jobs_{stamp}.json"
    csv_path = export_dir / f"yc_jobs_{stamp}.csv"
